...
(define (app work) 
    (if (null? work) '()
        (append (car work) (app (cdr work)))))

 (define (intersection-points ls)
    (if (= (length ls) 1) '()
        (append (app (map (lambda (x) (intersect (car ls) x)) (cdr ls))) (intersection-points (cdr ls))))) 

(define (distance-product pt ls-pt)
    (apply * (map (lambda (x) (dist x pt)) (remove ls-pt pt))))

(define (rate-points ls)
    (map (lambda (x) (list (distance-product x ls) x)) ls))


(define (partition1 pivot num-list)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition1 pivot (cdr num-list))))
	(if (< (caar num-list) pivot)
	    (list (cons (car num-list) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car num-list) (car (cdr split-of-rest))))))))

(define (quicksort1 num-list)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition1 (caar num-list) (cdr num-list))))
	(append (quicksort1 (car split)) 
		(list (car num-list)) 
		(quicksort1 (cadr split))))))
...
(define (clumped-points ls)
  (map (lambda (x) (flatten (cdr x))) (prefix-of-list (quicksort1 (rate-points ls)) (if (even? (length ls)) (/ (length ls) 2) (/ (- (length ls) 1) 2)))))
...
(define (average-point ls)
    (let ((avg (map (lambda (x) (/ x (length ls))) (apply generic-map + ls))))
...
(define (where-am-i rad loc)
    (quicksort1 (map (lambda (y) (best-estimate y)) (map (lambda (x) (generic-map list rad x)) (permutations loc)))))
