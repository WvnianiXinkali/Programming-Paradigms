                ...
                AccountAmount amount)
{
  ...
  pthread_mutex_lock(&bank->repLock);
  ...
  pthread_mutex_unlock(&bank->repLock);
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  pthread_mutex_lock(&bank ->atomLock);
  
  bank->workerDone++;

  if(bank->workerDone == numWorkers){

  int oldNumReports = rpt->numReports; Y;
  rpt->numReports = oldNumReports + 1; Y;


  for(int p = 0; p < numWorkers - 1; p++)
  sem_post(&bank->workerScheduler);

  bank->workerDone = 0;
  pthread_mutex_unlock(&bank ->atomLock);

  } else {

    pthread_mutex_unlock(&bank ->atomLock);

    sem_wait(&bank ->workerScheduler);
  }
